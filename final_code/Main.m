...
g = @(x)[x(1)+x(2)*t_step;x(2)+(Beta*(input_array - x(2))^2 + rho*A/m*(input_array-x(2))*(x(2)+input_array/2)...
         -tanh(x(2))*f/m)*t_step];  % nonlinear equation
...
for i = 1:time-1
    ...
    Velhat = Xhat_arr(2,i) + (Beta*(input_array - Xhat_arr(2,i))^2 + rho*A/m*(input_array-Xhat_arr(2,i))*(Xhat_arr(2,i)+input_array/2) ...
             - tanh(Xhat_arr(2,i))*f/m)*t_step;
    mu = [Poshat;Velhat];
    G = [1 t_step; 0 1 + (2*Beta*(Xhat_arr(2,i)-input_array) + rho*A/m*(input_array-2*Xhat_arr(2,i)-input_array/2) ...
        - sech(Xhat_arr(2,i))^2*f/m)*t_step];
    H = [1 0;0 1];
    sigma = G*sigma*G' + R;
...
hold on
plot(t,X_arr(1,:), 'k', 'linewidth', 2);
plot(t,Xhat_arr(1,:),'Color', [0, 0.2, 0.9], 'linewidth', 1.5);
plot(t,Xhat_ukf_arr(1,:),'Color', [0.1, 0.8, 0.1], 'linewidth', 1.5);
...
xlabel('Time [s]','FontSize',16)
hold on
plot(t,X_arr(2,:),'k', 'linewidth', 2);
plot(t,Xhat_arr(2,:),'Color', [0, 0.2, 0.9],'linewidth', 1.5);
plot(t,Xhat_ukf_arr(2,:),'Color', [0.1, 0.8, 0.1],'linewidth', 1.5);
legend('Ground truth','EKF', 'UKF','FontSize',14)
